  ...
  render () {
    ...
    if (attachment.get('type') === 'image') {
      content = media.map((image) =>
        <ImageLoader previewSrc={image.get('preview_url')} src={image.get('url')} width={image.getIn(['meta', 'original', 'width'])}  height={image.getIn(['meta', 'original', 'height'])} key={image.get('preview_url')} />
      ).toArray();
